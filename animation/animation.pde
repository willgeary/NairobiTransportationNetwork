...
String allTripsFile = "data/nairobi_tuesday.csv";
...
void setup() {
  ...
  Location nairobi = new Location(-1.210788, 36.7972003);
  int zoom = 12;
  map.zoomAndPanTo(zoom, nairobi);
...
void draw() {
  ...
  String date = new java.text.SimpleDateFormat("EEEE").format(new java.util.Date(epoch * 1000L));
  ...
  f = createFont("AppleSDGothicNeo-Light", 38, true);  // Loading font
  fill(0);
  textFont(f, 60);
  text("Informal Transportation", 40, 100);
  text("Networks in Nairobi, Kenya", 40, 160);

  
  f = createFont("AppleSDGothicNeo-Light", 36, true);  // Loading font
  textFont(f, 36);
  text("Typical Weekday Morning", 40, 280);
  ...
  textFont(f, 22);
  text("One Matatu", 72, 440);
  
  f = createFont("AppleSDGothicNeo-Light", 20, true);  // Loading font
  textFont(f, 22);
  text("@wgeary", 40, height-20);

  // Legend
  //fill(255, 255, 255);
  //stroke(1);
  fill(31,120,180, 200);
  ellipse(52, 435, 20, 20);
  //fill(0);
